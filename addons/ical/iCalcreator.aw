...
define( 'ICALCREATOR_VERSION', 'iCalcreator 2.2.8' );
/*********************************************************************************/
/*********************************************************************************/
/**
 * vcalendar class
 ...
 */
  function _cmpfcn( $a, $b ) {
    ...
    if(         !empty( $a->dtstart['value'] ) ||
                !empty( $b->dtstart['value'] )) {
      foreach( $this->_sortkeys as $key ) {
        if(      empty( $a->dtstart['value'][$key] ))  return -1;
        elseif(  empty( $b->dtstart['value'][$key] ))  return  1;
    ...
    if(          empty( $d ) && !empty( $b->duration['value'] ))
      $d = $b->duration2date();
    if(         !empty( $c ) || !empty( $d )) {
      if(        empty( $c ) && !empty( $d ))          return -1;
      if(       !empty( $c ) &&  empty( $d ))          return  1;
      foreach( $this->_sortkeys as $key ) {
        if    (  empty( $c[$key] ))                    return -1;
        elseif(  empty( $d[$key] ))                    return  1;
        if    (         $c[$key]
                     == $d[$key] )
          continue;
        if    ((  (int) $c[$key] ) <
    ...
    $c = (      !empty( $a->created['value'] )) ? $a->created['value'] : $a->dtstamp['value'];
    $d = (      !empty( $b->created['value'] )) ? $b->created['value'] : $b->dtstamp['value'];
    foreach( $this->_sortkeys as $key ) {
 ...
 */
  function parse() {
	...
	case "DTSTART":
	  ...
	  $day = substr($line, 6, 2);
	  $hours = substr($line, 9, 2);
	  $minutes = substr($line, 11, 2);
	  $seconds = "00";
	  $tz = substr($line, 15, 1);
	  $this->setProperty($propname, $year, $month, $day, $hours, $minutes, $seconds, $tz);
	  break;
        case 'ATTENDEE':
