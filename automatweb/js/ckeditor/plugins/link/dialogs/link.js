			...
			},
			{
				...
				[
					...
					[
						{
							type:'html',
							html:'<div style="height:40px" class="cke_dialog_ui_text" id="id_uiElement" role="presentation"><label for="link_id_textInput" id="id_label" class="cke_dialog_ui_labeled_label">Id</label><div role="presentation" class="cke_dialog_ui_labeled_content"><div role="presentation" class="cke_dialog_ui_input_text"><input type="text" aria-labelledby="id_label" size="38" id="link_id_textInput" class="cke_dialog_ui_input_text"></div></div></div><div style="height:40px" class="cke_dialog_ui_text" id="name_uiElement" role="presentation"><label for="link_name_textInput" id="name_label" class="cke_dialog_ui_labeled_label">Name</label><div role="presentation" class="cke_dialog_ui_labeled_content"><div role="presentation" class="cke_dialog_ui_input_text"><input type="text" size="38" id="link_name_textInput" class="cke_dialog_ui_input_text"></div></div></div>',
						},
						{
							type:'html',
							html:'<a id="cke_360_uiElement" aria-labelledby="cke_359_label" role="button" class="cke_dialog_ui_button" hidefocus="true" title="Search" href="javascript:search_links()" style="-moz-user-select: none;"><span class="cke_dialog_ui_button" id="cke_359_label">Search</span></a><div id="link_search_results" style="overflow:auto;height:140px;margin-top:10px;"></div>',
						}
					]
				},
			{
...
{
	var sid = document.getElementById('link_id_textInput');
	var sname = document.getElementById('link_name_textInput');
	if(sid.value != "" || sname.value != "")
	{
		...
		ajaxRequest.onreadystatechange = function(){
			if(ajaxRequest.readyState == 4){
				var ajaxDisplay = document.getElementById('link_search_results');
				ajaxDisplay.innerHTML = ajaxRequest.responseText;
			//	alert(ajaxRequest.responseText);
			}
		}
		
		var queryString = "?class=links&action=search_link&sid=" + sid.value + "&sname=" + sname.value;
...
function set_link(strLink,comment,id)
{
	// get current dialog
	var dialog = CKEDITOR.dialog.getCurrent();

	dialog.selectPage('advanced');
	// get the txtUrl element (found id using devtools plugin)
	var asdasd = dialog.getContentElement('advanced', 'advId');
	asdasd.setValue(id);
	...
	if(comment == 1)
	{
		...
		var tcmnt = dialog.getContentElement('target', 'linkTargetType');
		// set this input box to by variable
		tcmnt.setValue("_blank");
	}
	...
	// get the txtUrl element (found id using devtools plugin)
	var tUrl = dialog.getContentElement('info', 'url');
	// set this input box to by variable
