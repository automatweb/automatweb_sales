	...
	private function _add_cust_mgr($arr, $tree_inst)
	{
		...
		foreach($nms as $id => $nm)
		{
			$tree_inst->add_item("cmgr", array(
				...
				"url" => aw_url_change_var(crm_company::REQVAR_CATEGORY, null, aw_url_change_var("filt_p", null, aw_url_change_var("cmgr", $id,aw_url_change_var("pmgr", null,aw_url_change_var("area", null)))))
			));
		}
...
/* projektijuhid*/
		...
		foreach($tmp as $item)
		{
			if ($this->can("view", $item["proj_mgr"]))
			{
				$ids[] = $item["proj_mgr"];
			}
		}
		...
		foreach($nms as $id => $nm)
		{
			$tree_inst->add_item("pmgr", array(
				"id" => "pmgr_".$id,
				"name" => parse_obj_name($nm),
				"url" => aw_url_change_var(crm_company::REQVAR_CATEGORY, null, aw_url_change_var("filt_p", null, aw_url_change_var("pmgr", $id,aw_url_change_var("area", null))))
			));
		}
	...
	private function _add_areas($arr, $tree_inst)
	{
		$tree_inst->add_item(0, array(
			"id" => "areas",
			"name" => t("Piirkonnad"),
		...
		foreach($ol->arr() as $o)
		{
			$tree_inst->add_item("areas", array(
				"id" => $o->id(),
				"name" => $o->name(),
				"url" => aw_url_change_var(crm_company::REQVAR_CATEGORY, null, aw_url_change_var("area", $o->id(),aw_url_change_var("filt_p", null, aw_url_change_var("pmgr", $id))))
			));
		}
	...
	function _get_customer_categories_table(&$arr)
	{
		...
		elseif ($parent_category->is_a(crm_category_obj::CLID) or $parent_category->is_a(crm_company_obj::CLID))
		{
			...
			if($categories->count())
			{
				...
				$url = new aw_uri($user_inst->mk_my_orb($arr["request"]["action"], array(
					"group" => $arr["request"]["group"],
					"id" => $arr["request"]["id"],
				), $arr["request"]["class"]));
				...
				do
				{
					...
					$menu->add_item(array(
						"text" => t("Muuda"),
						"link" => $user_inst->mk_my_orb("change", array("id" => $category_id, "return_url" => get_ru()), "crm_category")
					));

					// get customer count
					$customer_count = new object_data_list(
						array(
							"class_id" => crm_company_customer_data_obj::CLID,
							"CL_CRM_COMPANY_CUSTOMER_DATA.RELTYPE_CATEGORY" => $category_id
						),
						array(
							crm_company_customer_data_obj::CLID => array(new obj_sql_func(obj_sql_func::COUNT, "count" , "*"))
						)
					);
					$customer_count = $customer_count->arr();
					$customer_count = reset($customer_count);
					$customer_count = $customer_count["count"];

					$url->set_arg(crm_company::REQVAR_CATEGORY, $category_id);
					$modifiedby = $user_inst->get_person_for_uid($category->modifiedby());
					$table->define_data(array(
						"oid" => $category->id(),
						...
						"modifiedby" => $modifiedby ? $modifiedby->name() : $category->modifiedby()
					));
				}
				...
				if ($parent_category->is_a(crm_category_obj::CLID))
				{
					$this->layoutinfo["category_list_container"]["area_caption"] = sprintf(t("Kategooria '%s' alamkategooriad"), $parent_category->name());
				}
				elseif ($parent_category->is_a(crm_company_obj::CLID))
				{
					$this->layoutinfo["category_list_container"]["area_caption"] = t("Peataseme kategooriad");
				}
			}
			else
			{
