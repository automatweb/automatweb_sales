...
HANDLE_MESSAGE_WITH_PARAM(MSG_STORAGE_NEW, CL_REALESTATE_ROWHOUSE, on_create)
HANDLE_MESSAGE_WITH_PARAM(MSG_STORAGE_DELETE, CL_REALESTATE_PROPERTY, on_delete)

@classinfo syslog_type=ST_REALESTATE_ROWHOUSE relationmgr=yes no_comment=1 no_status=1 trans=1 maintainer=voldemar
...
class realestate_rowhouse extends realestate_property
{
	const AW_CLID = 992;

	function realestate_rowhouse()
	{
		$this->init(array(
			"tpldir" => "applications/realestate_management/realestate_rowhouse",
			"clid" => CL_REALESTATE_ROWHOUSE
		));
	}
