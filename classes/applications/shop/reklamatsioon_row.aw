...
@default table=aw_reklamatsioon_row
...
class reklamatsioon_row extends class_base
{
	...
	{
		$this->init(array(
			"tpldir" => "applications/shop/reklamatsioon_row",
			"clid" => CL_REKLAMATSIOON_ROW
		));
	}
	...
	function do_db_upgrade($t, $f)
	{
		if ($f == "")
		{
			$this->db_query("CREATE TABLE aw_reklamatsioon_row(aw_oid int primary key)");
			return true;
		}
