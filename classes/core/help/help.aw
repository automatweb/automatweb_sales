...
class help extends aw_template implements orb_public_interface
{
	function help()
	{
		$this->tpl_init("help");
...
/** shows a help browser for a class
	...
	**/
	function browser($arr)
	{
		...
		$atc = new add_tree_conf();//XXX: teine instants?
		...
		$help_orb_name = !empty($arr["group"]) ? "grouphelp":"classhelp";
		$this->vars(array(
			...
			"help_content" => $this->mk_my_orb($help_orb_name, array(
				"clid" => $arr["clid"],
				"grpid" => empty($arr["group"]) ? "" : $arr["group"]
			)),
		));
		return $this->parse();
	}
